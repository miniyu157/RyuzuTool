...
Public Class FirmUpdate
    ...
    Private Sub FirmUpdate_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        DownRyujinx.StartInfo.Arguments = "(URL)" & Label1.Tag & "(SavePath)" & Application.StartupPath & "(SaveName)Firm.7z(Job)Yuzu 固件更新(x)" & Me.Left & "(y)" & Me.Top & "(End)"
        DownRyujinx.Start()
    ...
    Private Sub C7zAndFile()
        ...
        If Directory.Exists(OpenRyu.GetValue("InstallFolder") & "\user\nand\system\Contents\registered\") Then
            For Each SF In My.Computer.FileSystem.GetFiles(OpenRyu.GetValue("InstallFolder") & "\user\nand\system\Contents\registered\")
                File.Delete(SF)
            Next

            For Each SD In My.Computer.FileSystem.GetDirectories(OpenRyu.GetValue("InstallFolder") & "\user\nand\system\Contents\registered\")
        ...
        JieFirm.StartInfo.Arguments = "x """ & Application.StartupPath & "\Firm.7z"" -o""" & OpenRyu.GetValue("InstallFolder") & "\user\nand\system\Contents\registered\" & """"
        JieFirm.StartInfo.WindowStyle = ProcessWindowStyle.Hidden
        JieFirm.Start()
        JieFirm.WaitForExit()

        InstallFirmware(OpenRyu.GetValue("InstallFolder") & "\user\nand\system\Contents\registered")
